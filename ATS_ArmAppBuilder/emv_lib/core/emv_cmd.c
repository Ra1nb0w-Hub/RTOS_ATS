    ...
    while (1)
    {
        *response_len = response_capacity;
        ...
        if (g_emv_session.interface_type == EMV_INTERFACE_CONTACTLESS)
        {
            ...
            iRet = g_emv_reader.picc_transceive_apdu(command, command_len, response, response_len);
            if (iRet < 0)
            {
        ...
        else
        {
            ...
            iRet = g_emv_reader.icc_transceive_apdu(command, command_len, response, response_len);
            if (iRet < 0)
            {
