...
int emv_terminal_set_reader(EMVReaderInterface *reader)
{
    ...
    memcpy(&g_emv_reader, reader, sizeof(EMVReaderInterface));
...
int emv_step_detect_card(void)
{
    ...
    ret = g_emv_reader.poll_card(&g_emv_session.interface_type, g_emv_session.request.check_interface_timeout);
    if (ret != EMV_OK) {
        g_emv_reader.close();
    ...
    if (g_emv_session.interface_type == EMV_INTERFACE_CONTACT)
    {
        ...
        EmvLog("ICC power on...");
        ret = g_emv_reader.icc_power_on(atr, &atr_len);
        if (ret != EMV_OK) {
            g_emv_reader.close();
