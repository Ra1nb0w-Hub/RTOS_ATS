...
static bool s_uart_rx_enabled = false;
...
static void ats_uart_init_once(bool enable_rx)
{
    uint32_t ctrl_value;

    if (s_uart_initialized)
    {
        if (!enable_rx || s_uart_rx_enabled)
        {
            return;
        }
    }
    ...
    if (enable_rx)
    {
        ...
        s_uart_rx_enabled = true;
    }

    ats_uart_write_reg(ATS_UART_BASE, ATS_UART_CTRL_OFFSET, 0U);
    ats_uart_write_reg(ATS_UART_BASE, ATS_UART_BAUDDIV_OFFSET, ATS_UART_BAUD_DIVISOR);
    ats_uart_write_reg(ATS_UART_BASE, ATS_UART_CTRL_OFFSET, ctrl_value);

    s_uart_initialized = true;
}

static bool ats_uart_try_read_byte(uint8_t *byte)
{
    if (byte == NULL)
    {
        return false;
    ...
    if ((ats_uart_read_reg(ATS_UART_BASE, ATS_UART_STATE_OFFSET) & ATS_UART_STATE_RXFULL) == 0U)
    {
        return false;
    }

    *byte = (uint8_t)(ats_uart_read_reg(ATS_UART_BASE, ATS_UART_DATA_OFFSET) & 0xFFU);
    return true;
}

static int ats_uart_transport_write(const uint8_t *data, uint16_t length)
{
    uint16_t index;
    uint16_t batch;
    ...
    ats_uart_init_once(false);

    index = 0U;
    while (index < length)
    {
        while ((ats_uart_read_reg(ATS_UART_BASE, ATS_UART_STATE_OFFSET) & ATS_UART_STATE_TXFULL) != 0U)
        {
        }
        ...
        {
            ats_uart_write_reg(ATS_UART_BASE, ATS_UART_DATA_OFFSET, (uint32_t)data[index]);
            ++index;
            ++batch;
        }
    }
...
{
    (void)user_data;
    return ats_uart_transport_write(data, length);
}
...
int ats_rpc_transport_read(uint8_t *byte, uint32_t timeout_ms, void *user_data)
{
    ...
    for (;;)
    {
        if (ats_uart_try_read_byte(byte))
        {
            return ATS_EC_OK;
        }
